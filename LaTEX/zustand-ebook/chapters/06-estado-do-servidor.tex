...
\ilustracaopendente{6-1-lista-desatualizada}{Cena no DevPanel: duas pessoas
comparam a página de Usuários com o modal de convite aberto ao lado. Os dois
lugares exibem quantidades diferentes de usuários, enquanto uma terceira pessoa
aponta para a API no quadro e pergunta qual versão é a atual. Mostrar que ambas
as telas leram cópias locais em momentos diferentes.}
...
\ilustracaopendente{6-2-fluxo-query-cache}{Diagrama com API à esquerda, cache do
TanStack Query no centro e página de Usuários mais modal de convite à direita.
A primeira tela solicita a chave ``users'', o cache busca a API e guarda a
resposta; a segunda tela recebe a mesma entrada. Uma seta pontilhada representa
uma atualização em segundo plano sem apagar a lista já exibida.}
...
\ilustracaopendente{6-3-cliente-servidor}{Quadro comparativo dividido ao meio.
No lado ``estado do cliente'', mostrar tema, Drawer e rascunho de filtro ligados
ao Zustand. No lado ``estado do servidor'', mostrar usuários e relatórios ligados
ao cache do TanStack Query e à API. No centro, um componente do DevPanel combina
as duas fontes sem duplicar os dados.}
